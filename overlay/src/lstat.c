...
{
    /* Function scope on purpose: filename may end up pointing here, and a
       buffer declared inside the block below would be gone by the time
       lstat_rel reads it. */
    char abs_filename[FAKECHROOT_PATH_MAX];

    debug("lstat(\"%s\", &buf)", filename);

    if (filename != NULL && !fakechroot_localdir(filename)) {
