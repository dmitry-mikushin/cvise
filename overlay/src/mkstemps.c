...
wrapper(mkstemps, int, (char * template, int suffixlen))
{
    ...
    /* No redirection: the caller gets its buffer back with the final
       name in it, and a redirected name would not be the name of the
       file it can then open. These create files in TMPDIR, outside the
       tree under reduction, so there is nothing to redirect anyway. */
    }
    ...
    if ((fd = nextcall(mkstemps)(tmpptr, suffixlen)) == -1 || !*tmpptr) {
        goto error;
    }
