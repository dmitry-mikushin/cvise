...
# What is offered for deletion. The whole reducible tree, not one file: a
# fixture's real footprint is spread across headers, templates and sibling
# translation units, and a per-file run can only delete what one file happens to
# contain.
REDUCIBLE = ('cpp/src', 'cpp/include')
...
def configure_slot_zero(repo: Path, worktree: Path, builds: Path, log: Path) -> None:
    """Configure from the project's own preset, and nowhere else.

    A build directory configured any other way grades the candidate under flags
    the project does not use, which is a reduction of a program nobody builds.
    """
    ...
    proc = docker(
        ...
        '-w', SRC, BUILD_IMAGE,
        'cmake', '--preset', 'default', '-B', BUILD_IN_CONTAINER,
...
docker run --rm \\
    ...
    -v "$ROOT/trees/$slot/src:{SRC}/{SUBMODULE_IN_SRC}/cpp/src:ro" \\
    -v "$ROOT/trees/$slot/include:{SRC}/{SUBMODULE_IN_SRC}/cpp/include:ro" \\
...
def main() -> int:
    ...
    configure_slot_zero(repo, worktree, builds, root / 'configure_0.log')
