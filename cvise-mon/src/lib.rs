//! Finding the live reduction and measuring it.
//!
//! Shared by the two programs that face it: cvise-mon, which watches, and
//! cvise-ctl, which acts. Kept in one place because the hard part -- deciding
//! WHICH container is the reduction -- is done by its mounts, and both have
//! already been given the wrong answer by its name and by its image tag.
...
pub fn gather(previous: Option<&Snapshot>, elapsed: Option<f64>) -> Option<Snapshot> {
...
pub fn plain(snap: &Snapshot) -> String {
...
/// What to say when there is nothing to watch or to stop.
pub fn nothing_to_watch() -> i32 {
