...
def check_script(project: 'Project', name: str, path: Path, jobs: int = 1) -> Path:
    ...
    The build gets this job's share of the machine, and that is neither a
    throttle nor a free-for-all. C-Vise has already decided how many candidates
    run at once, so the two numbers have to be multiplied, not each taken to be
    the whole machine.

    MEASURED, both ways round. With no -j at all, every one of 44 jobs got a
    ninja willing to use all 88 cores: thousands of concurrent C++ compilers at
    some 250 MB each, load average 230, and the reduction's own cgroup
    OOM-killed cc1plus 88 seconds in. With -j 1 the machine was safe and half
    idle -- load 50 of 88 -- and every candidate that touched dozens of files
    compiled them one after another, so 16 of them hit the 300 s timeout in
    eight minutes. A share each uses the machine exactly once.
    ...
    path.write_text(
        ...
        f'cmake --build {build} -j {max(1, jobs)} 2>&1 || exit $?\n'
