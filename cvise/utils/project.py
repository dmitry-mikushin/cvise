...
def check_script(project: 'Project', name: str, path: Path, target: str | None = None) -> Path:
    ...
    targeted = f' --target {target}' if target else ''
    # Whether a candidate was actually compiled, written down as the verdict is
    # made. A reduction that accepts a candidate it never built is the one
    # failure that looks like success, and afterwards there is nothing left to
    # examine: the job's directory is gone, and recreating the moment from
    # outside recreates a different moment. Seven such reconstructions in a row
    # all held while the run itself did not.
    witness = Path(build).parent / 'cvise-verdicts.log'
    invalidate = Path(__file__).parent / 'invalidate.py'
    # The binary the test runs, located before the build so that its identity is
    # fixed and only its mtime can move underneath. Without a target there is
    # nothing to look for, and the guard below says so rather than comparing two
    # absences and calling them equal.
    locate = (
        f'b=$(find {build} -type f -name {shlex.quote(target)} 2>/dev/null | head -1)\n'
        if target
        else 'b=\n'
    )
    path.write_text(
        ...
        + locate
        + 'was=$(stat -c %Y-%s "$b" 2>/dev/null || echo none)\n'
        # Objects that depended on what this candidate deleted, removed before
        # the build. Without this ninja sees no live edge naming the vanished
        # file and reports nothing to do, and the candidate is graded by the
        # previous one's binary. MEASURED: 99 verdicts in one run with 966
        # headers deleted, correctly hidden, and not one compilation.
        f'killed=$({shlex.quote(sys.executable)} {shlex.quote(str(invalidate))} '
        f'{build} "$PWD/.cvise-delta" 2>/dev/null || echo 0)\n'
        f'cmake --build {build}{targeted} > .cvise-build.log 2>&1; rc=$?\n'
        'now=$(stat -c %Y-%s "$b" 2>/dev/null || echo none)\n'
        f'printf "build=%s killed=%s rebuilt=%s dir=%s\\n" '
        '"$rc" "$killed" "$([ "$was" = "$now" ] && echo NO || echo yes)" "$PWD" '
        f'>> {witness} 2>/dev/null\n'
        'if [ "$rc" -ne 0 ]; then cat .cvise-build.log; exit "$rc"; fi\n'
        # If objects were removed, the binary has to come back newer. It cannot
        # legitimately be unchanged: something rebuilt it from a source this
        # program cannot see -- a cache that kept the old object, an overlay
        # that never reached the compiler -- and a verdict on that is a verdict
        # about the previous candidate. Guarded on "objects were removed" so
        # that an untouched tree, which rightly rebuilds nothing, is not refused
        # for doing exactly what it should.
        'if [ "${killed:-0}" -gt 0 ]; then\n'
        '  if [ -z "$b" ] || [ "$was" = none ]; then\n'
        # Nothing to compare against, so the check above proves nothing. Saying
        # so is the only honest answer: "I could not tell" must not be recorded
        # as "it passed", which is the very substitution this guard exists for.
        '    echo "cvise: $killed objects were removed but the binary under test was not"\n'
        '    echo "cvise: found before the build, so this candidate cannot be shown to"\n'
        '    echo "cvise: have been compiled at all."\n'
        '    exit 125\n'
        '  fi\n'
        '  if [ "$was" = "$now" ]; then\n'
        '    echo "cvise: $killed objects were removed and the test binary did not change;"\n'
        '    echo "cvise: this candidate would have been judged by another one\x27s binary."\n'
        '    exit 125\n'
        '  fi\n'
        'fi\n'
