...
def check_script(project: 'Project', name: str, path: Path, lock: Path) -> Path:
    ...
    So the builds are queued instead. Every job takes the same lock before
    building and releases it after, which means one build at a time with the
    whole machine behind it, rather than dozens of builds each with a sliver.
    No CPU is given up by this -- the same work is done in the same total time,
    just in an order -- and what it removes is the multiplication: one build's
    worth of compilers and one build's worth of memory, instead of forty-four.

    The lock lives outside the trees the overlay covers, and that is not
    incidental: a lock file inside them would be copied into each job's delta,
    every job would take its own private lock, and the queue would be a queue of
    one. Everything would look right and nothing would be serialised.

    The test is not in the queue. It runs against the binary in this job's own
    delta, which no other job can touch, and holding the machine for it would
    serialise the part that was already safe to do at once.
