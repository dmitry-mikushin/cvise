...
def check_script(project: 'Project', name: str, path: Path) -> Path:
    ...
    One thread, and that is not a throttle. C-Vise has already decided how many
    things run at once -- one job per core -- so a job that hands its build the
    whole machine counts the same parallelism twice. MEASURED: `cmake --build`
    with no -j gives every one of 44 jobs a ninja with 88 threads, which is
    thousands of concurrent C++ compilers at some 250 MB each; the load average
    reached 230 and the reduction's own cgroup OOM-killed cc1plus 88 seconds in.
    The ceiling did its job -- the machine was never at risk -- but the run was
    over.
    ...
    path.write_text(
        ...
        f'cmake --build {build} -j 1 2>&1 || exit $?\n'
